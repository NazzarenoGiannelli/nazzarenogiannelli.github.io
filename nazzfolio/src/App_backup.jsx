...
const PersonalWebsite = () => {
  ...
  const expertise = [
    { image: 'https://via.placeholder.com/400x300', title: 'Architectural Visualization', description: 'Creating photorealistic 3D renderings and animations' },
    { image: 'https://via.placeholder.com/400x300', title: 'Unreal Engine Specialist', description: 'Authorized Instructor for Unreal Engine' },
    { image: 'https://via.placeholder.com/400x300', title: 'Real-Time Design', description: 'Crafting interactive architectural experiences' }
  ];
        ...
        <section id="expertise" className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {expertise.map(({image, title, description}, index) => (
              <div key={index} className="bg-[#121212] hover:border-[#4037db] flex flex-col items-center transform transition-all border border-gray-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {projects.map(({image, link, title, description}, index) => (
              <a href={link} target='_blank' rel='noreferrer' key={index} className="bg-[#121212] hover:border-[#4037db] overflow-hidden transform transition-all duration-300 ease-in-out hover:scale-105 border border-gray-800">
                <img src={image} alt="Product preview" className="w-full h-40 object-cover" />
                <div className="p-6">
                  <h3 className="text-xl mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>{title}</h3>
                  <p className="text-gray-400 mb-4">{description}</p>
                  <a href={link} target='_blank' rel='noreferrer' className="bg-[#4037db] text-white hover:border-2 border-[#4037db] hover:bg-inherit font-bold py-2 px-6 rounded-none transition-colors duration-200 animate-fade-in border">View Details</a>
                </div>
              </a>
              ))}
          </div>
